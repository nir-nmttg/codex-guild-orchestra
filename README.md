...
運用上のメタ認知は、別 runtime ではなく既存の Guild contract へ写像します。
これは自己意識ではなく、作業中の known facts、unknowns、assumptions、evidence、confidence、risk、verification status を監視し、次の行動を制御するための仕組みです。
補助文書は [docs/metacognitive-runtime.md](docs/metacognitive-runtime.md) を参照してください。
