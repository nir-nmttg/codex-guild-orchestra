...
Rootは直接assignmentを作り、custom agentはすべてterminalです。RootはSol/highを既定とし、利用者がhigh/xhigh/maxから選べます。maxはRootのまれな明示overrideだけに限定し、routine evalやsubagentでは使いません。現在のsubagent deploymentは`guildmaster`だけSol/xhigh、他の5.6 roleはSol/highを維持しつつ、bounded roleではTerra/high、`sage`ではLuna/highをrole別live非劣性で評価します。`courier`は5.3-Spark/xhighを維持します。
