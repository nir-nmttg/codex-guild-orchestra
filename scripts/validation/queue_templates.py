...
def validate_queue_templates() -> None:
    ...
    require("metacognitive_state:" in request_text, "request.yaml example は metacognitive_state を含めてください。")
    require("- metacognitive_state" in request_text, "request.yaml handoff required_evidence は metacognitive_state を含めてください。")
    require("confidence_percent" in request_text and "verification_status" in request_text, "request.yaml metacognitive_state example は confidence_percent / verification_status を含めてください。")
    ...
    for key in ("id", "quest_id", "rank", "objective", "intent_analysis", "success_criteria", "implementation_strategy", "metacognitive_state", "control_decision", "owned_scope", "authority", "boundaries", "autonomy_budget", "research_plan", "validation_expectations", "trial_expectations", "escalation_triggers", "evidence_required", "status"):
    ...
    validate_metacognitive_state(assignment.get("metacognitive_state"), "adventurer_assignment.assignment.metacognitive_state")
    ...
    require({"objective", "intent_analysis", "metacognitive_state", "implementation_strategy", "owned_scope", "authority", "boundaries", "autonomy_budget", "trial_expectations"} <= set(sequence(assignment_handoff.get("required_evidence"), "adventurer_assignment.assignment.handoff_sufficiency.required_evidence")), "adventurer_assignment handoff required_evidence が不足しています。")
    ...
    for key in ("id", "quest_id", "worker_id", "role", "kind", "rank", "objective", "intent_analysis", "success_criteria", "non_goals", "focus", "metacognitive_state", "control_decision", "authority", "boundaries", "known_context", "autonomy_budget", "research_plan", "advisor_consultation", "output_requirements", "escalation_triggers", "evidence_required", "status"):
    ...
    validate_metacognitive_state(cartographer_assignment.get("metacognitive_state"), "cartographer_assignment.assignment.metacognitive_state")
    ...
    for key in ("id", "quest_id", "depth", "focus", "objective", "intent_analysis", "success_criteria", "metacognitive_state", "control_decision", "authority", "boundaries", "trial_checks", "depth_guardrails", "autonomy_budget", "research_plan", "decision_options", "evidence_required", "status"):
    ...
    validate_metacognitive_state(trial.get("metacognitive_state"), "inquisitor_trial.trial.metacognitive_state")
    ...
    validate_metacognitive_state(cartographer_report.get("metacognitive_state"), "cartographer_report.report.metacognitive_state")
    ...
    for rel in ("template/.agents/orchestra/queue/templates/adventurer_report.yaml", "template/.agents/orchestra/queue/templates/inquisitor_report.yaml"):
        ...
        for key in ("id", "quest_id", "worker_id", "status", "summary", "metacognitive_state", "control_decision", "validation_evidence", "research_evidence", "confidence", "risks", "evidence_refs"):
            require(key in report, f"{rel}.report.{key} が必要です。")
        validate_metacognitive_state(report.get("metacognitive_state"), f"{rel}.report.metacognitive_state")
        ...
        if rel.endswith("adventurer_report.yaml"):
            ...
            require({"changed_files", "decisions_made", "intent_alignment", "metacognitive_state", "control_decision", "validation_evidence", "research_evidence", "risks"} <= set(sequence(handoff.get("required_evidence"), f"{rel}.report.handoff_sufficiency.required_evidence")), f"{rel}.report.handoff_sufficiency.required_evidence が不足しています。")
        ...
        if rel.endswith("inquisitor_report.yaml"):
            ...
            require({"decision", "findings", "intent_coverage", "metacognitive_state", "control_decision", "validation_evidence", "advisor_dialogue_synthesis", "reviewer_synthesis", "finding_dispositions", "risks"} <= set(sequence(handoff.get("required_evidence"), f"{rel}.report.handoff_sufficiency.required_evidence")), f"{rel}.report.handoff_sufficiency.required_evidence が不足しています。")
