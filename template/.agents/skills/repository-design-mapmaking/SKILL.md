...
- `cartographer` に渡す mapmaking assignment の `id`、`quest_id`、`objective`、`focus`、`autonomy_budget`、`evidence_required`
- advisor 利用を検討するための subassignment budget
...
4. Root セッションが、目的、`success_criteria`、`non_goals`、`authority`、`boundaries`、`known_context`、`autonomy_budget`、`escalation_triggers`、`evidence_required` を含む Quest Charter を短く整理する。
...
9. `cartographer` は `autonomy_budget.subassignments` が 1 以上で focus が authority / boundaries 内に収まる場合、狭い focus の read-only `advisor` 利用を既定で検討する。使わない場合は理由を advisor synthesis に残す。
...
- advisor synthesis、または advisor を使わなかった理由
