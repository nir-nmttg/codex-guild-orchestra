...
ギルド規約ルート直下の `repositories/<repo>` として管理され、Root が明示した `target_repo_root` で示される対象リポジトリについて、bounded verification assignmentを受けた`adventurer`が実装内容と参考情報から動作確認項目を作り、実際に確認するためのワークフローです。Rootはtarget、authority、snapshot、success criteriaを固定し、実装差分の読み取り、テスト、curl、CLI、ログ確認、browser操作、debugを直接代替しません。
...
5. `adventurer`は確認項目を優先度つきで整理する。最低限、主要正常系、主要異常系、回帰リスク、UIがある場合のin-app browser確認を含める。
6. `adventurer`は実行方法を決める。repo既存のtest / lint / build / dev server / Docker / script / curl / browser操作を優先し、依存追加や外部networkは人間確認なしに行わない。
...
10. `adventurer`は`browser:control-in-app-browser`など利用可能なin-app browser操作を使い、対象画面を開いて確認する。画面がない場合は、確認した代替surfaceと、画面確認が非該当である理由を明記する。
...
13. `adventurer`は確認項目、参考リンク、実行した確認、in-app browserのevidence、失敗や未確認、残リスク、追加Questの要否をreportとしてRootへ返す。Rootはreportとsnapshotをgateし、再実装、Trial、停止、完了の次actionを決める。
