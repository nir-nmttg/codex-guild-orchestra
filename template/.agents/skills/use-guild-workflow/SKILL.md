...
10. Trial は risk-based に選ぶ。uncertainty、coupling、blast radius、safety risk、validation result を見て `none`、`self_check`、`peer_review`、`focused_trial`、`multi_focus_trial`、`safety_gate` から選択する。
11. Critical / Major の不足がある場合は、必要最小の追加 Quest として返す。Root は未解消の不足を完了扱いにしない。
12. 完了時は、Quest Charter、担当 report、Trial evidence、実行した検証、未検証範囲、残リスクを集約して短く報告する。Ledger 反映が必要な場合は `courier` に任せる。
