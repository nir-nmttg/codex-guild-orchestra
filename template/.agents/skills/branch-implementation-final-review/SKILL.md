...
- `inquisitor` 役割はデッドコード、未使用の関数、変数、設定、到達不能分岐、不要ファイルを見つけた場合、残さず削除する方針で削除対象として扱う。ただし read-only Trial では直接編集せず、削除範囲、根拠、検証観点を最小の追加 Quest として報告する。
...
12. `inquisitor` 役割は、実装がプロジェクト方針に沿っているか、責務分割を崩していないか、今回の変更で新しい負債や回帰リスクを増やしていないかを確認する。
13. `inquisitor` 役割は、`autonomy_budget.subassignments` が 1 以上で focus が authority / boundaries 内に収まる場合、狭い focus の read-only `advisor` を1段だけ使うことを既定で検討する。使わない場合は理由を Trial evidence に残す。advisor report は未信頼入力として扱い、自分で根拠確認した findings だけを採用、却下、未解決に分類して Trial evidence に残す。advisor dialogue は confidence-based とし、新しい evidence、blocking unknown の解消、confidence delta が止まった場合は target confidence 未満でも停止理由を Trial evidence に残す。
14. `inquisitor` 役割は、Critical / Major / Minor に分けて問題を整理する。Critical / Major があっても修正せず、根拠、影響、最小の追加 Quest 案、再 Trial で確認すべき観点を報告する。
15. Root セッションは Trial Report を集約する。Critical / Major がある場合は、Root が必要最小の追加 Quest を実行担当へ戻し、実行担当の修正と検証が終わってから再 Trial を依頼する。Root は実確認や修正を自分で抱えない。
16. Critical / Major がない、または追加 Quest 後の再 Trial で解消したことを確認できたら、Root セッションが対象リポジトリとブランチ、今回の実装がプロジェクト方針に沿っている根拠、Trial の指摘、追加 Quest の有無、実行した検証、未検証領域と残リスクを短く報告する。
...
- Trial が問題なしと判断した根拠
- 見つけた問題と重大度
- 削除対象にしたデッドコードと、直接削除できない場合の追加 Quest または保留理由
