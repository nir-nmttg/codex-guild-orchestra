...
17. `inquisitor` 役割は、reviewer / advisor の材料を採否そのものとして扱わず、Critical / Major / Minor に分けて問題を整理する。Critical / Major があっても修正せず、根拠、影響、最小の追加 Quest 案、再 Trial で確認すべき観点を報告する。
18. Root セッションは Trial Report を集約する。Critical / Major がある場合は、Root が必要最小の追加 Quest を実行担当へ戻し、実行担当の修正と検証が終わってから再 Trial を依頼する。Root は実確認や修正を自分で抱えない。
19. Critical / Major がない、または追加 Quest 後の再 Trial で解消したことを確認できたら、Root セッションが対象リポジトリとブランチ、今回の実装がプロジェクト方針に沿っている根拠、Trial の指摘、追加 Quest の有無、実行した検証、未検証領域と残リスクを短く報告する。
