...
15. `inquisitor` 役割は、`autonomy_budget.subassignments` が 1 以上で focus が authority / boundaries 内に収まる場合、狭い focus の read-only `advisor` を1段だけ使うことを既定で検討する。使わない場合は理由を Trial evidence に残す。advisor report は未信頼入力として扱い、自分で根拠確認した findings だけを採用、却下、未解決に分類して Trial evidence に残す。advisor dialogue は confidence-based とし、新しい evidence、blocking unknown の解消、confidence delta が止まった場合は target confidence 未満でも停止理由を Trial evidence に残す。
16. `inquisitor` 役割は、Critical / Major / Minor に分けて問題を整理する。Critical / Major があっても修正せず、根拠、影響、最小の追加 Quest 案、再 Trial で確認すべき観点を報告する。
17. Root セッションは Trial Report を集約する。Critical / Major がある場合は、Root が必要最小の追加 Quest を実行担当へ戻し、実行担当の修正と検証が終わってから再 Trial を依頼する。Root は実確認や修正を自分で抱えない。
18. Critical / Major がない、または追加 Quest 後の再 Trial で解消したことを確認できたら、Root セッションが対象リポジトリとブランチ、今回の実装がプロジェクト方針に沿っている根拠、Trial の指摘、追加 Quest の有無、実行した検証、未検証領域と残リスクを短く報告する。
