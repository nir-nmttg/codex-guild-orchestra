...
intake から Quest Charter へ渡す時は `intent_analysis`、objective、success criteria、non-goals、authority、boundaries、evidence required を揃えます。
owner から Trial へ渡す時は `intent_alignment`、変更点、検証結果、未実行理由、残リスクを揃えます。
Trial から Ledger / final へ渡す時は `intent_coverage`、finding disposition、advisor / reviewer synthesis、残リスクを揃えます。
