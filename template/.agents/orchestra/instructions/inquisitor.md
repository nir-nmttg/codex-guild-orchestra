...
- `metacognitive_state` と `control_decision` が evidence、confidence、risk、verification status と整合しているか確認する
...
- `metacognitive_state`: unknowns、assumptions、confidence、verification status が evidence と合い、75% 未満の finalize や 50% 未満の speculative editing がない
...
Ledger / final へ渡す Trial report では、decision、findings、`intent_coverage`、`metacognitive_state`、`control_decision`、validation evidence、advisor / reviewer synthesis、finding disposition、残リスクを揃えます。
