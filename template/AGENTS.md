...
非 trivial な Quest では、担当は `metacognitive_state` と `control_decision` を維持し、confidence-based control signal として次の行動を選びます。
...
confidence が 75% 未満なら finalize せず、追加 evidence、追加検証、diff 確認、必要なら `metacognitive_controller` を使います。confidence が 50% 未満なら speculative editing を止め、`revise_plan` として task contract と不足 evidence を再構成します。人間確認条件に触れる時だけ `stop_for_user_approval` を選びます。
...
関連する漏れは plan、verification checklist、security review、residual risk、または認知ミス補正 memory へ入れます。

## Subagent Trigger Policy

trivial edit では subagent を使いません。
uncertainty、risk、context overload を下げる時だけ使います。
`metacognitive_controller` は confidence が 75% 未満、重要 unknown が残る、scope が広がる、test が繰り返し失敗する、plan 変更が必要、long-running または high-risk な時に検討します。
...
- `metacognitive_controller`: 作業中の metacognitive_state、confidence、unknowns、verification status から次アクションを推薦する read-only 制御監視担当
