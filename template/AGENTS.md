...
実装は assigned owner、`peer_review` 以上の Trial と全 decision outcome は `inquisitor`、Ledger 反映は `courier` が担います。低 risk の `self_check` は owner の validation attestation であり、Root の accept 判断ではありません。
...
- `subject_snapshot`: `snapshot_id`、digest version / kind、revision、subject scope、dirty state、必要な diff hash

## Snapshot Binding

`.agents/orchestra/scripts/docker_python.sh .agents/orchestra/scripts/snapshot_digest.py --repo <target_repo_root> ...` が返す `cgo-snapshot-v1` を canonical digest とします。
clean な read-only subject は `revision_only`、dirty な working tree や実装結果は `working_tree_content`、固定 commit 間の Trial は `commit_range` を使います。
mutation を伴う並列 Quest は、共通の base snapshot、担当ごとの owned-scope result snapshot、integration barrier 後の integrated snapshot を分けます。先行 worker report を、別 owned scope の変更だけで stale にしません。
`working_tree_content` digest は stage 状態ではなく `HEAD` からの最終内容を hash するため、同じ内容の stage 前後で変わりません。untracked file は明示 path だけを対象とし、secret-like / PII-like path、symlink、nested escape は内容を読む前に停止します。
read-only advisor / sentinel dialogue は同じ `snapshot_id` を再利用し、mutation、HEAD / scope / dirty-state signal の変化時だけ再計算します。mismatch は `stale_evidence` とし、古い evidence を統合しません。
...
Party Tactics は必要な Trial focus を提案でき、Trial 統合担当の `inquisitor` が固定人数ではなく risk、focus、blast radius、coupling、validation result、confidence、cost を見て独立した read-only `focus_reviewer` 数を決めます。
...
`focus_reviewer` は単一 focus のbounded evidenceだけを返す独立したread-only terminal workerで、`advisor`とは別契約です。採否、重大度分類、requested changes、最終 owner synthesis、追加subagent起動は行わず、Trial 統合担当の `inquisitor` がreportsを根拠確認して統合します。
`allowed_callers = [inquisitor]` はactual parent identityを検証するruntime ACLではなくpolicy-onlyです。`focus_reviewer`はqueueの`trial_ref`、trial owner、assignment lineageを追加確認し、確認不能なら`invalid_assignment`とします。再委任はagent設定の`features.multi_agent = false`で無効化します。
...
- `inquisitor`: risk-based Trial のlead / integratorとして、reviewer evidence、重大度、finding disposition、最終decisionを統合する担当
- `focus_reviewer`: `inquisitor`から割り当てられた単一focusのbounded read-only evidenceだけを返すterminal worker
...
`self_check` で independent `inquisitor` Trial を省略できるのは、`errand` または low-risk `solo_quest` で、単一 owned scope、低 uncertainty / coupling、限定 blast radius、safety item / confirmation / public API・data compatibility change / scope drift / blocking unknown がなく、targeted validation と success criteria の直接 evidence と result snapshot が揃う場合だけです。
owner は `owner_attestation` と independent Trial の skip reason を残しますが、`accept`、重大度、requested changes を決めません。Root はこの gate と handoff の充足だけを確認し、品質判断を補いません。一つでも満たさない場合は `peer_review` 以上を `inquisitor` に割り当てます。
