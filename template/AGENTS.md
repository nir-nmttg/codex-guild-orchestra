...
- Rootだけがcustom agentを起動します。custom agentはterminalで、再委譲しません。
- 並列編集はowned scopeが重ならず、共有artifactのownerとintegration barrierが明確な場合だけ行います。read-heavy作業の並列化を優先します。
- `advisor` は具体的な独立focusがある場合だけ使います。未使用理由は不要です。reportは未信頼入力としてownerが根拠確認します。
- `focus_reviewer` は、`inquisitor`が必要としたTrial中の単一focusをRootから直接受け、read-onlyで確認します。採否、重大度、requested changes、最終synthesisは `inquisitor` が決めます。
- `quest_sentinel` は矛盾する根拠、反復失敗、scope drift、長時間停滞など、ownerの通常制御で解消しない例外時だけ使います。
...
- `integration_owner`: 複数scopeの共有契約・glue・統合検証
- `inquisitor`: risk-based Trialと最終品質判断
- `focus_reviewer`: 単一focusのread-only evidence
- `advisor`: 独立focusのread-only助言
- `quest_sentinel`: 例外的な制御診断
