...
phase oneはmodelとeffortを同時に変更せず、bounded roleのSol/Terra/Lunaをすべてhighで比較します。Rootはhigh/xhigh、`guildmaster`は現行xhighとhigh、`inquisitor`は現行highとxhighを比較します。maxはroutine matrixへ含めず、Rootが明示選択した場合の例外設定としてだけ扱います。旧Rootの5.5 effortは継承値だったため、比較controlをhighへ正規化した仮定をmanifestに記録します。
