...
3. boundary、safety、sequencing の狭い focus で sage を使う場合、`guildmaster` が根拠を再確認し、採用、却下、未解決の disposition を strategy に統合します。
...
9. Trial lead の `inquisitor` が stable snapshot に対して `multi_focus_trial`、必要なら `safety_gate` を行います。必要な単一focusはRootへ提案し、Rootが独立roleの`examiner`を直接起動します。`inquisitor`はarchitecture、security、compatibility、regression、validationのreportsを根拠確認して統合します。
...
- sage / examiner の根拠確認と finding disposition がある
