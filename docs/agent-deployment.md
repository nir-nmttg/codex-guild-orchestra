...
`.codex/agents` に展開される Codex subagent は次の 8 つです。
...
| `metacognitive_controller` | `.codex/agents/metacognitive_controller.toml` | `gpt-5.5` | `read-only` | `high` | metacognitive_state、confidence、unknowns、verification status を監視し、次アクションだけを推薦する |
...
| `inquisitor` | 3 | Trial focus ごとに独立確認できる |
| `advisor` | 4 | focus 限定の read-only 助言だけを返す |
| `metacognitive_controller` | 2 | confidence、unknowns、verification status の制御判断だけを返す |

`advisor` は terminal worker です。
実装、採否、Ledger 反映、追加 subagent 起動は行わず、owner が根拠確認した findings だけを synthesis に採用します。
`metacognitive_controller` も read-only の terminal worker で、実装や採否ではなく `control_decision` の推薦だけを返します。

focus reviewer 数は固定しません。
軽微で局所的な変更は追加 reviewer 0..1、`multi_focus_trial` や安全境界、検証不足、広い blast radius がある変更は `autonomy_budget.subassignments` と `workers.inquisitor.max_parallel` の小さい方を上限に分割します。
