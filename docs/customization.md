...
- `metacognitive_control`: 作業中の state、confidence、risk、verification status から次アクションを決める制御契約
...
- confidence threshold、omission detection、subagent trigger の説明
- 役割指示の表現
...
`metacognitive_controller` は read-only の制御監視担当で、実装、採否、Ledger 反映は行いません。
confidence が 75% 未満、重要 unknown が残る、scope drift、繰り返す failed check、high-risk 作業では起動を検討します。
