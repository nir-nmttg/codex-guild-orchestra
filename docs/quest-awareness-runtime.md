...
通常のcontrolはownerが行います。`quest_sentinel`は、contradictory evidence、repeated failure、scope drift、長時間停滞など、owner内で解消できない例外だけをread-onlyで診断します。実装、採否、Ledger/Git/外部状態更新は行いません。
